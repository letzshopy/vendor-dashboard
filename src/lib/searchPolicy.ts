...
export type SearchScope = "products" | "orders" | "customers";
...
export function searchScope(value: string | null): SearchScope {
  return value === "orders" || value === "customers" ? value : "products";
}
...
export function customerSearchResult(value: unknown): SearchResult | null {
  if (!isRecord(value)) return null;
  const id = safeId(value.id);
  if (!id) return null;
  const name = [
    safeText(value.first_name, 100),
    safeText(value.last_name, 100),
  ].filter(Boolean).join(" ").trim();
  const email = safeText(value.email, 254);
  const username = safeText(value.username, 100);
  const subParts = [email, username].filter(Boolean);
  const target = email || name || username;

  return {
    id,
    label: name || email || username || "Customer",
    ...(subParts.length ? { subLabel: subParts.join(" • ") } : {}),
    url: `/customers?search=${encodeURIComponent(target)}`,
  };
}
