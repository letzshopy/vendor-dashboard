...
const STORAGE_KEY = (k: MenuKey) => `ls_menu_${k}_single_v1`;
...
export default function MenuLayoutPage() {
  ...
  const currentDef = useMemo(
    () => MENUS.find((m) => m.key === menuKey) || MENUS[0],
    [menuKey]
  );
  ...
  async function saveAndSync() {
    ...
    try {
      ...
      if (currentDef.saveMode === "location") {
        ...
        if (!id) {
          notify(`Cannot save — missing menu ID for “${currentDef.wpName}”.`);
          return;
        }
        body.menu_id = id;
      }
  ...
  function addPage(parentId: string | null = null) {
    ...
    if (!p) return;
    setItems((prev) =>
      insertNodeUnderParent(prev, parentId, {
        id: uid(),
        type: "page",
        title: p.name,
        url: p.url,
  ...
  function addCategory(parentId: string | null = null) {
    ...
    setItems((prev) =>
      insertNodeUnderParent(prev, parentId, {
        ...
        title: c.name,
        refId: c.id,
        url: `/product-category/${slugify(c.name)}`,
        children: [],
      })
    );
    setSelectedCatId("");
  ...
  function addCustom(parentId: string | null = null) {
  ...
  function maxDepthForNode(id: string, depth = 0, arr = items): number {
    for (const node of arr) {
      if (node.id === id) return depth;
      const found = maxDepthForNode(id, depth + 1, node.children || []);
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
        <section className="overflow-hidden rounded-[26px] border border-slate-200/80 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
          <div className="border-b border-slate-100 bg-gradient-to-r from-[#faf7ff] via-white to-[#f4fbff] px-4 py-4 md:px-5">
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">
              Add root items first, then use Add Child inside the tree.
            </p>
          </div>
          ...
          <div className="space-y-4 p-4 md:p-5">
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => selectedPageId && addPage()}
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => selectedCatId && addCategory()}
                  className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-600 text-white shadow-sm"
                >
                  <Plus className="h-5 w-5" />
                </button>
        ...
        <section className="overflow-hidden rounded-[26px] border border-slate-200/80 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
          <div className="border-b border-slate-100 bg-gradient-to-r from-[#faf7ff] via-white to-[#f4fbff] px-4 py-4 md:px-5">
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">
              Parent dropdown makes nesting simple. Indentation shows submenu
              depth clearly.
            </p>
          </div>
          ...
          <div className="p-4 md:p-5">
            {loadingMenu ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-10 text-center text-sm text-slate-500">
                No items in this menu yet. Add root items from the left panel.
              </div>
            ) : (
              <div className="space-y-3">
                {rebuildTreeIds(items).map((item, index) => (
                  <Row
                    key={item.id}
                    item={item}
                    level={0}
