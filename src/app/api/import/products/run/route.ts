...
async function productPayload(woo: AxiosInstance, row: CsvRow): Promise<{
  ...
  const promotionalPricing = [
    ...
    rowValue(row, "sale from", "date_on_sale_from"),
    rowValue(row, "sale to", "date_on_sale_to"),
  ];
  if (promotionalPricing.some(Boolean)) {
    throw new ImportRequestError(
      "Sale pricing is managed through Sale Events and cannot be imported with products.",
    );
  }
