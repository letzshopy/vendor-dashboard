...
function pickAddress(o: WCOrder) {
  const a = (o.shipping && (o.shipping.first_name || o.shipping.address_1)) ? o.shipping : (o.billing || {});
...
export default async function PackSlipsPage({
  ...
  try {
    if (!ids.length) error = "No order IDs provided.";
    else orders = await getOrders(ids);
  } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="no-print p-3 text-sm text-slate-600">
        Pack Slips (A4 portrait) — 2 per page (top/bottom). Extra buffer to keep the footer on the same page; added safe right padding for the order number.
      </div>

      {error ? (
        <div className="sheet">
          <div className="bg-white rounded border p-6 text-slate-700">{error}</div>
        </div>
      ) : (
        <>
                      ...
                      <div className="slip" key={o.id}>
                        <div>
                          <div className="hdr">
                            ...
                            <div className="meta"><b>Order:</b> #{o.number || o.id}</div>
                          </div>

                          <div className="addr">
                            <div><b>Ship To:</b> {a.name || "-"}</div>
                            {a.lines.map((ln, i) => <div key={i}>{ln}</div>)}
                            {a.phone ? <div><b>Mobile:</b> {a.phone}</div> : null}
                          </div>
                                  ...
                                  <tr key={li.id}>
                                    <td>
                                      <div style={{ wordBreak: "break-word" }}>
                                        {li.name}
                                        {li.sku ? <span className="sku"> ({li.sku})</span> : null}
                                      </div>
                                    </td>
