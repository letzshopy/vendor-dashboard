...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Detail mode disables Save until something changes
  const isDetailMode = (searchParams.get("mode") || "").toLowerCase() === "detail";

  // ✅ Presence-based saved flag (like Media page: just `?saved`)
  ...
  // No shipping class UI (module to be redesigned)
  ...
  // ✅ Auto-clear ?saved after banner shows (same pattern as Media)
  useEffect(() => {
    if (!hasSaved) return;
    const t = setTimeout(() => {
  ...
  const isDirty = useMemo(() => {
    if (!isDetailMode) return true; // normal edit mode: Save always enabled (as before)
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      // ✅ Mirror Media behavior: set local msg and add a presence-only "?saved"
      setMsg("Saved successfully.");
      const params = new URLSearchParams(searchParams.toString());
      params.set("saved", ""); // presence flag only
      router.replace(`?${params.toString()}`, { scroll: false });

      if (isDetailMode) setInitialSnapshot(snapshotFromState()); // reset dirty baseline
    } catch (e: any) {
  ...
  return (
    <main className="p-6 max-w-5xl mx-auto">
      ...
      {/* ✅ Flash banners — show if local msg OR ?saved present (like Media) */}
      {(msg || hasSaved) ? (
        <div className="mb-3">
          <Flash message={msg || "Saved successfully."} />
        </div>
      ) : null}
      {err ? (
        <div className="mb-3">
          <Flash message={err} kind="error" />
        </div>
      ) : null}
      ...
      <form onSubmit={submit} onKeyDown={swallowEnter} className="space-y-6">
        ...
        <section className="border rounded-lg p-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="w-full border rounded px-3 py-2 text-sm" value={sku} onChange={(e) => setSku(e.target.value)} />
            </div>
            <div>
        ...
        {ptype === "grouped" && (
          <section className="border rounded-lg p-4">
                ...
                {groupSelected.map((gid) => (
                  <div key={gid} className="flex items-center justify-between px-2 py-1">
                    ...
                    <button
                      ...
                      onClick={() => setGroupSelected((s) => s.filter((x) => x !== gid))}
                    >
                      Remove
                    </button>
                  </div>
            ...
            <div className="text-xs text-slate-500 mt-2">Save changes to persist the group.</div>
          </section>
        )}
